...
def update_data():
    if not st.session_state.candidates: return
...
if st.session_state.rt_results:
    ...
    with col2:
        ...
        fig.add_trace(go.Scatter(x=buy_idx, y=df_chart.loc[buy_idx, 'Low'] * 0.97, mode='markers+text', 
                                 marker=dict(symbol='star', size=15, color='royalblue'), text="💙", textposition="bottom center", name="매수"))
        fig.add_trace(go.Scatter(x=sell_idx, y=df_chart.loc[sell_idx, 'High'] * 1.03, mode='markers+text', 
                                 marker=dict(symbol='star', size=15, color='#FF4B4B'), text="❤️", textposition="top center", name="매도"))
